...
class Bot {
  constructor(config){
    ...
    this.addCommand("_reload_plugins", (message, args)=>{
      try{
  ...
  addCommand(command, method){
    this.commandProcessor.addCommand(command, method);
  }

  removeCommand(command){
    this.commandProcessor.removeCommand(command);
  }

  addListener(event, method){
    this.client.addListener(event, method) 
  }

  removeListener(event, method){
    this.client.removeListener(event, method) 
  }
