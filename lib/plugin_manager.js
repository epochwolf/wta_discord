    ...
    try{
      ...
      for(let [event, array] of plugin.listeners){
        for(let method of array){
          try{
            this.bot.addListener(event, method);  
          }catch(e){
            console.log(`Failed to addListener(${event}, ${method})`);
            throw e;
          }
        }
      }
      for(let [command, method] of plugin.commands){
        try{
          this.bot.addCommand(command, method);
        }catch(e){
          console.log(`Failed to addCommand(${command}, ${method})`);
          throw e;
        }
      }
      plugin.__load();
  ...
  unload(name){
    ...
    try{
      ...
      for(let [event, array] of plugin.listeners){
        for(let method of array){
          try{
            this.bot.removeListener(event, method);
          }catch(e){
            console.log(`Failed to removeListener(${event}, ${method})`);
            //throw e;
          }
        }
      }
      for(let [command, method] of plugin.commands){
        try{
          ...
          console.log(`Failed to removeCommand(${command})`);
          //throw e;
        }
      }
      plugin.__unload();
